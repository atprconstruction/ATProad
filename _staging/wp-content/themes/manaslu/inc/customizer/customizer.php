 ...
 */
function manaslu_customize_register( $wp_customize ) {
	...
	if ( isset( $wp_customize->selective_refresh ) ) {
		$wp_customize->selective_refresh->add_partial(
			...
			array(
				...
				'render_callback' => 'manaslu_customize_partial_blogname',
			)
		);
		$wp_customize->selective_refresh->add_partial(
			'blogdescription',
			array(
				...
				'render_callback' => 'manaslu_customize_partial_blogdescription',
			)
		);
	}

	/*Get default values to set while building customizer elements*/
	$default_options = manaslu_get_default_customizer_values();
	/* Header Background Color*/
	$wp_customize->add_setting(
	    'manaslu_options[header_bg_color]',
	    array(
	...
	$wp_customize->add_control(
	    new WP_Customize_Color_Control(
	        ...
	        'manaslu_options[header_bg_color]',
	        array(
	            'label' => __('Header Background Color', 'manaslu'),
	            'section' => 'colors',
	            'type' => 'color',
        		'priority' => 1,
	...
	require_once get_template_directory() . '/inc/customizer/theme-options/front-page-popular.php';
	...
	$wp_customize->add_section( 'pro__section', array(
		'title'       => '' . esc_html__( 'View PRO Version', 'manaslu' ),
		'priority'    => 2,
		'description' => sprintf(
			...
			__( '<div class="upsell-container">
					...
					<ul class="upsell-features">
                            <li>
                            	...
                            	<div class="description">Having different tastes and preferences might be tricky for users, but not with Manaslu onboard. It has an intuitive and catchy interface which allows you to change <strong>fonts, colors or layout sizes</strong> in a blink of an eye.</div>
                            </li>

                            <li>
                            	<h4>Adaptive Layouts For Your Posts</h4>
                            	<div class="description">Whether your featured image is in portrait or landscape mode, Manaslu takes care of it by changing the post layout to provide the right fit.</div>
                            </li>
			...
			sprintf( '<a href="%1$s" target="_blank" class="button button-primary">%2$s</a>', esc_url( manaslu_get_pro_link() ), esc_html__( 'View Manaslu PRO', 'manaslu' ) )
		),
	) );
	$wp_customize->add_setting( 'manaslu_style_view_pro_desc', array(
		'default'           => '',
		'sanitize_callback' => '__return_true',
	) );
	$wp_customize->add_control( 'manaslu_style_view_pro_desc', array(
		'section' => 'pro__section',
		'type'    => 'hidden',
	) );
}
add_action( 'customize_register', 'manaslu_customize_register' );
 ...
 * @return void
 */
function manaslu_customize_partial_blogname() {
	bloginfo( 'name' );
}
 ...
 * @return void
 */
function manaslu_customize_partial_blogdescription() {
 ...
 */
function manaslu_customize_preview_js() {
    wp_enqueue_script( 'manaslu-customizer', get_template_directory_uri() . '/assets/js/customizer.js', array( 'customize-preview' ), aJiMA_S_VERSION, true );

}
add_action( 'customize_preview_init', 'manaslu_customize_preview_js' );
 ...
 * @since 1.0.0
 */
function manaslu_customizer_control_scripts(){
    wp_enqueue_style('manaslu-customizer-css', get_template_directory_uri() . '/assets/css/customizer.css');
    wp_enqueue_script('manaslu-customizer-controls', get_template_directory_uri() . '/assets/js/customizer-admin.js', array('jquery', 'jquery-ui-sortable', 'customize-controls') );
}
add_action('customize_controls_enqueue_scripts', 'manaslu_customizer_control_scripts', 0);

/**
 * Generate a link to the Manaslu Lite info page.
 */
function manaslu_get_pro_link() {
	return 'https://www.themeinwp.com/theme/manaslu-pro/';
}